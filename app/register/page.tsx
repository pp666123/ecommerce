...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("註冊成功！", { description: response.message });
      router.push("/login");
    } catch (error) {
      // 安全地處理 unknown 型別的錯誤
      const errorMessage =
      ...
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 p-8 rounded-3xl shadow-xl shadow-black/5 border border-zinc-200 dark:border-zinc-800">
        ...
        {/* <div className="mt-6">
          <Button
            type="button"
            variant="outline"
            className="w-full h-12 rounded-xl font-bold border-zinc-300 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors flex items-center justify-center gap-2"
          >
