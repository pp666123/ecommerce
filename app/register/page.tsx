...
export default function RegisterPage() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      toast.success("註冊成功！", { description: "您的帳號已建立，請重新登入。" });
      router.push("/login");
    }, 1500);
  };
    ...
    <div className="w-full min-h-[80vh] flex items-center justify-center px-4 py-12 bg-zinc-50 dark:bg-zinc-950">
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 p-8 rounded-3xl shadow-xl shadow-black/5 border border-zinc-200 dark:border-zinc-800">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-zinc-900 dark:text-white">建立帳號</h1>
          <p className="text-zinc-500 mt-2 text-sm">加入我們，解鎖專屬潮流體驗</p>
        </div>

        <form onSubmit={handleRegister} className="space-y-5">
          <div className="space-y-2">
            <label className="text-sm font-bold text-zinc-700 dark:text-zinc-300">使用者名稱</label>
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-zinc-700 dark:text-zinc-300">設定密碼</label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
